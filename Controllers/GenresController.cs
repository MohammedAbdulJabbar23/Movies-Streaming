...
namespace MovieApp.API.Controllers
{
    ...
    public class GenresController : ControllerBase
    {
        ...
        public IActionResult PartialUpdateGenre(Guid genreId, JsonPatchDocument<GenreDTO> patchDoc)
        {
            ...
            // Apply the patch and handle errors
            patchDoc.ApplyTo(genrePatchDto, error =>
            {
                _logger.LogError($"Error applying JSON patch: {error.ErrorMessage}");
            });

            // Validate the patched object
            if (!TryValidateModel(genrePatchDto))
            {
